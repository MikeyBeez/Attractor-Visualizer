...
class ResultsAnalyzer:
    ...
    def load_patterns(self, patterns_path):
        ...
        for section in sections:
            ...
            elif current_section and len(lines) > 0:
                # These are pattern entries
                for line in lines:
                    if line.startswith(('1.', '2.', '3.', '4.', '5.', '6.', '7.', '8.', '9.')):
                        parts = line.split('(score:')
                        if len(parts) == 2:
                            pattern = parts[0].split('. ', 1)[1].strip()
                            score = float(parts[1].rstrip(')').strip())
    ...
    def _create_attractor_dashboard(self):
        ...
        if 'ngrams' in self.patterns and self.patterns['1-grams']:
        ...
        if not is_notebook:
            ...
            while True:
                ...
                if choice == '1':
                    ...
                    if not self.patterns:
                        print("Pattern analysis results not available")
                        continue
